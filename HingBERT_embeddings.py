...
# Initialize the tokenizer and model for HingBERT
tokenizer = BertTokenizer.from_pretrained('l3cube-pune/hing-mbert')
model = BertModel.from_pretrained('l3cube-pune/hing-mbert')
...
def generate_embeddings(df):
    embeddings_list = []
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Generating HingBERT Embeddings"):
        tweet = row['Tweet']
        faux_label = row['FAUX']
        tweet_id = row['ID']
...
with open('Test_HingBERT_embeddings.pkl', 'wb') as f:
    pickle.dump(embeddings, f)

print("Embeddings saved to 'Test_HingBERT_embeddings.pkl'")
